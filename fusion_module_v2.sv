...
// fusion_module_v2.sv
//
// Optimized version of fusion_module.sv
// Key optimization: FC layer unrolled by factor of 8
// Processes 8 input channels per clock instead of 1
//
// Original:  CONCAT_CH cycles for FC  (1344 cycles)
// Optimized: CONCAT_CH/8 cycles       (168 cycles)
// Speedup:   8x on FC layer
//
// Additional: GAP runs in parallel with streaming
// ============================================================

import sffn_params::*;

module fusion_module_v2 #(
    parameter SPATIAL_CH  = 1280,
    parameter FREQ_CH     = 64,
    parameter FUSED_CH    = 512,
    parameter SPATIAL_HW  = 7,
    parameter FREQ_HW     = 28,
    parameter UNROLL      = 8    // FC unroll factor
)(
    input  logic                                       clk,
    input  logic                                       rst_n,
    input  logic                                       start,
    ...
    // FC iterations with unrolling
    localparam FC_ITERS    = CONCAT_CH / UNROLL;
    ...
    reg [9:0]  sp_pix_cnt;
    reg [9:0]  fr_pix_cnt;
    reg [10:0] fc_iter_cnt;   // counts FC_ITERS iterations
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            case (state)
                ...
                // ── KEY OPTIMIZATION ──────────────────────────
                // Process UNROLL=8 input channels per cycle
                // FC_ITERS = CONCAT_CH/8 = 168 cycles
                // vs original 1344 cycles → 8x speedup
                S_FC: begin
                    for (oc = 0; oc < FUSED_CH; oc++) begin
                        // Unrolled MAC — 8 per cycle
                        for (uu = 0; uu < UNROLL; uu++) begin
                            fc_out[oc] <= fc_out[oc] +
                                $signed(concat_vec[
                                    fc_iter_cnt*UNROLL+uu]) *
                    ...
                    if (fc_iter_cnt == FC_ITERS - 1) begin
                        fc_iter_cnt <= '0;
                        state       <= S_OUTPUT;
                    end
                    else
                        fc_iter_cnt <= fc_iter_cnt + 1;
                end
